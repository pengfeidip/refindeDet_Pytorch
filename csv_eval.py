...
parser.add_argument('--trained_model',
                    default='weights/RefineDet320_CSV_115000.pth', type=str,
...
def test_net(save_folder, net, cuda, dataset, transform, top_k,
    ...
    output_dir = get_output_dir('refineDet_320_120000', set_type)
...
def load_preds(classes):
    preds = []
    #  把detection的结果整理一下
    for class_name in classes:

        with open(os.path.join(devkit_path, "results/det_test_" + class_name + ".txt")) as detect_file:
...
def evaluate_detections(box_list, output_dir, dataset):
    ...
    preds = load_preds(classes)

    with open(gt_file) as f:
        labels = list(csv.reader(f))

    print("Computing the mAp ing...... please wait a moment")
    ap = ComputemAP(preds=preds, labels=labels, class_list=classes, iou_thresh=0.5)
...
if __name__ == '__main__':
    ...
    dataset = CSVDataset(csv_file='./csv/voc/07_test.csv',
                         classes_file='./csv/voc/classes.csv',
                         transform=BaseTransform(int(args.input_size),
                                                 dataset_mean))

    net = build_refinedet('test', 320, dataset.num_classes)
